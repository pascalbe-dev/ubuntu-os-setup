...
# docker compose
alias dcd='docker compose -f $(find . | grep compose.yaml | head -1) down'
alias dcu='docker compose -f $(find . | grep compose.yaml | head -1) up -d'
...
alias mvsr='mvn spring-boot:run -Dspring-boot.run.profiles=local,flyway'
